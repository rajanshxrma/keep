...
mlx-lm is an optional `[mlx]` extra (see router.mlx_available) -- when it
isn't installed, every non-tool turn falls back to a single-turn on-device
call (router.run_afm_plain) instead, with no history replay.
"""

from langchain_core.messages import AIMessage, BaseMessage, HumanMessage

from keep.agent import build_agent
...
class Conversation:
    ...
    def ask(self, prompt: str) -> str:
        ...
        if not mlx_available():
            # No message history without mlx-lm installed -- this fallback
            # exists for the "optional extra not present" case, not as a
            # primary multi-turn path. See router.run_afm_plain.
            return run_afm_plain(prompt)
