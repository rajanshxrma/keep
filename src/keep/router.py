...
Two backends CAN be available, with very different tradeoffs (measured, see
...
mlx-lm is an OPTIONAL `[mlx]` extra, not a base dependency (see
`mlx_available()`) -- Keep ships without it by default so a plain
`pip install keep-assistant` never triggers a multi-GB Hugging Face
download, keeping the "nothing leaves the machine" claim true on first run.
Every plain-chat call site checks `mlx_available()` and falls back to
`run_afm_plain` (still fully on-device, just no local MLX model) when the
...
def mlx_available() -> bool:
    """Whether the optional `[mlx]` extra (mlx-lm + transformers) is installed.

    Keep ships without it by default -- the on-device Apple Foundation Model
    handles plain chat when mlx isn't present, so a fresh `pip install
    keep-assistant` (no extras) never triggers an mlx-lm/Hugging Face model
    download, keeping the "nothing leaves the machine, ever" claim true even
    on first run.
    """
...
def run_afm_plain(prompt: str) -> str:
    """Plain (non-tool) generation via the on-device Apple Foundation Model.

    Used only when the optional `[mlx]` extra isn't installed. Single-turn --
    unlike run_mlx it doesn't replay a message history, since this path exists
    for the "mlx not installed" case rather than as a primary chat path; a
    fresh ChatAppleFoundationModels session per call keeps this function
    simple and matches the same "shorter, accurate system prompt" reasoning
    as run_mlx below (agent.py's tool-describing INSTRUCTIONS don't apply here).
    """
    llm = ChatAppleFoundationModels(instructions=_MLX_SYSTEM_PROMPT)
