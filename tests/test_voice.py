...
def test_listen_transcribes_real_speech():
    ...
    # silent 15s stall was found 2026-07-09 and partially fixed 2026-07-10
    # (transcription now runs in a spawned subprocess -- see voice.py's
    # module docstring for the full trail). If this test fails fast (not
    # a long stall), that's ordinary noisy-environment misrecognition, not
    # the old bug -- rerun in a quiet room before suspecting a regression.
    for _ in range(3):
        subprocess.Popen(["bash", "-c", 'sleep 0.5 && say -r 135 "call the dentist"'])
        text = listen()
        # "contains", not exact match -- same reasoning as keep's
